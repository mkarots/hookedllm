...
class TestRules:
    ...
    def test_model_rule_matches(self):
        ...
        call_input = CallInput(model="gpt-4", messages=[Message(role="user", content="test")])
        context = CallContext()
    ...
    def test_model_rule_no_match(self):
        ...
        call_input = CallInput(
            model="gpt-3.5-turbo", messages=[Message(role="user", content="test")]
        )
        context = CallContext()
...
class TestExecutor:
    ...
    async def test_execute_after_hook(self):
        ...
        await executor.execute_after([(my_hook, None)], call_input, call_output, context)
    ...
    async def test_execute_hook_with_rule(self):
        ...
        await executor.execute_after([(my_hook, rule)], call_input, call_output, context)
    ...
    async def test_hook_failure_is_isolated(self):
        ...
        await executor.execute_after(
            [(failing_hook, None), (working_hook, None)], call_input, call_output, context
        )
